...
import { getFirestore, collection, addDoc, query, orderBy, onSnapshot, serverTimestamp, doc, setDoc, updateDoc, deleteDoc, increment, arrayUnion, arrayRemove, getDoc } from "https://www.gstatic.com/firebasejs/11.10.0/firebase-firestore.js";
    ...
    <article class="post glass ${p.pinned ? "pinned" : ""}">
      ...
      <div>
        <div class="post-topline">
          <div class="post-meta">
            ${p.pinned ? `<span class="pinned-tag"><span class="material-icons">push_pin</span>Pinned</span>` : ""}
        ...
        <div class="post-actions">
          ...
          ${currentUser && ALLOWED.has(currentUser.email?.toLowerCase()) && p.authorEmail === currentUser.email.toLowerCase()
            ? `<button class="action-btn" data-pin="${p.id}"><span class="material-icons">${p.pinned ? "push_pin" : "push_pin"}</span>${p.pinned ? "Unpin" : "Pin"}</button>` : ""}
          ${currentUser && ADMINS.has(currentUser.email?.toLowerCase())
            ? `<button class="action-btn delete-btn" data-delete="${p.id}" title="Delete post"><span class="material-icons">delete</span>Delete</button>` : ""}
        </div>
      </div>
    </article>`;
  }).join("");
...
function toast(message, type="info"){
  ...
  box.innerHTML = `<span class="material-icons">${type==="error" ? "error" : "check_circle"}</span><span>${esc(message)}</span>`;
...
function isAdmin(){
  return !!currentUser && ADMINS.has((currentUser.email || "").toLowerCase());
}
...
async function showProfile(email){
  ...
  profile.innerHTML = `
    <div class="modal-card glass profile-card">
      <button class="close icon-btn"><span class="material-icons">close</span></button>
